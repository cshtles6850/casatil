...
export default function AboutUsZh(){return <main lang="zh-CN">
  <section className="page-hero"><div className="container"><div className="breadcrumb"><Link href="/zh-cn">首页</Link><span>›</span><span>关于我们</span></div><span className="eyebrow">服务运营信息</span><h1>关于 Cappadocia Airport Shuttle</h1><p className="lead">我们专注于开塞利机场（ASR）、内夫谢希尔机场（NAV）与卡帕多奇亚中心住宿区之间的预订制机场接送。</p></div></section>
  <section className="section page-content-section"><div className="container content-grid"><article className="prose">
    <section className="content-section"><h2>机场拼车是主要服务</h2><p>主要服务覆盖格雷梅、于尔居普、乌奇希萨尔、阿瓦诺斯、奥塔西萨和恰武辛。希望独立车辆的旅客也可以选择 Mercedes Vito 或 Sprinter 私人接送。</p><p>网站把 ASR 和 NAV 的路线分开，让旅客在预订前看到对应距离、正常公路时间和私人车辆价格。</p></section>
    <section className="content-section"><h2>不是把卡帕多奇亚当成一个统一下客点</h2><p>洞穴酒店坡路、城堡边住宿、河两岸酒店和普通城镇道路的车辆条件不同。因此具体路线页面会说明该机场与该住宿区真正有关的接送信息。</p></section>
    ...
    <section className="content-section"><h2>旅行社运营</h2><p>Cappadocia Airport Shuttle 由 <strong>{SITE.company}</strong> 运营，TURSAB 编号 <strong>{SITE.tursab}</strong>。</p></section>
