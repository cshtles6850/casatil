...
function zhRoutePage(en:SeoPage):SeoPage {
  ...
  const sections:ContentSection[] = arrival ? [
    ...
    { heading:`预订 ${za.code} 到${town}时，酒店名称要写完整`, paragraphs:[`${p.special} 对 ${za.code} → ${town} 的接机安排来说，提前确认准确住宿入口可以减少抵达后再次沟通路线的时间。`, `以下住宿名称只是为了帮助识别${town}的地点，并不是酒店排名。从 ${za.code} 抵达时，即使你的酒店不在名单中，只要位于支持区域，也可以提交接送请求。`], bullets:p.hotels },
    ...
    { heading:`${town}送机预订中常见的住宿名称`, paragraphs:[`下面列出的住宿名称用于帮助识别${town}实际接车地点，并不是酒店排名，也不代表大型车辆一定能停在每家酒店正门。即使你的住宿不在名单中，只要位于支持区域，也可以填写完整酒店名称申请接送。`], bullets:p.hotels },
