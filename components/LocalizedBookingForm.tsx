...
const pickupTimeNotice: Record<NewLocale, string> = {
  es: 'La hora de recogida se organizará según los datos de su vuelo y será confirmada por nosotros.',
  'pt-BR': 'O horário de busca será definido de acordo com os dados do seu voo e confirmado por nós.',
  ko: '픽업 시간은 항공편 정보에 따라 정해지며 별도로 안내해 드립니다.',
  ja: 'お迎え時間はフライト情報に基づいて設定し、当社より改めてご案内します。',
};

const expandMissingWithoutTime: Record<NewLocale, string> = {
  es: 'Selecciona la localidad e introduce la fecha del vuelo y el nombre completo del hotel para continuar.',
  'pt-BR': 'Selecione a cidade e informe a data do voo e o nome completo do hotel para continuar.',
  ko: '계속하려면 지역을 선택하고 항공편 날짜와 숙소 전체 이름을 입력하세요.',
...
const contactCopy: Record<NewLocale, {
  ...
  phoneCountryWarning: string; phoneConfirmTitle: string; phoneConfirmText: string; phoneFix: string; phoneContinue: string;
}> = {
  es: {
    ...
    phoneCountryWarning: 'Parece que falta el prefijo internacional. Revísalo antes de continuar (por ejemplo, +34, +44, +90).',
    phoneConfirmTitle: 'Comprueba tu número de teléfono',
    phoneConfirmText: 'Parece que este número no incluye el prefijo internacional:',
    phoneFix: 'Corregir número',
    phoneContinue: 'Continuar de todos modos',
  },
  'pt-BR': {
    ...
    phoneCountryWarning: 'Parece que está faltando o código do país. Confira antes de continuar (por exemplo, +55, +44, +90).',
    phoneConfirmTitle: 'Confira seu número de telefone',
    phoneConfirmText: 'Este número parece não incluir o código do país:',
    phoneFix: 'Corrigir número',
    phoneContinue: 'Continuar mesmo assim',
  },
  ko: {
    ...
    emailLine: '연락 이메일',
    phoneCountryWarning: '전화번호에 국가 코드가 없는 것 같습니다. 계속하기 전에 확인해 주세요 (예: +82, +44, +90).',
    phoneConfirmTitle: '전화번호를 확인해 주세요',
    phoneConfirmText: '입력한 번호에 국가 코드가 없는 것 같습니다:',
    phoneFix: '번호 수정',
    phoneContinue: '그대로 계속',
  },
  ja: {
    ...
    emailLine: '連絡先メール',
    phoneCountryWarning: '電話番号に国番号がない可能性があります。続行する前に確認してください（例：+81、+44、+90）。',
    phoneConfirmTitle: '電話番号をご確認ください',
    phoneConfirmText: '入力された番号には国番号がないようです：',
    phoneFix: '番号を修正',
    phoneContinue: 'このまま続行',
  },
};
...
export function LocalizedBookingForm({ locale, compact=false, initialAirport='kayseri', initialDirection='airport-hotel', initialTown='' }: { locale:NewLocale; compact?:boolean; initialAirport?:Airport; initialDirection?:Direction; initialTown?:string }) {
  ...
  function updateWhatsApp(value:string){setWhatsapp(value);phoneCountryConfirmedRef.current=false;setPhoneConfirmPending(false)}
  function continueWithCurrentPhone(){phoneCountryConfirmedRef.current=true;setPhoneConfirmPending(false);formRef.current?.requestSubmit()}
  function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault(); if(submittingRef.current)return; if(!confirmed)return; if(!whatsapp.trim()){setStatus(contact.contactRequired);return;} if(!companyArrangedPickup&&getBookingTiming(firstTransferDate,firstTransferTime)==='past'){setStatus(`${timingCopy.past} ${timingCopy.pastStrong}`);return;} if(journey==='round-trip'&&returnOrderInvalid){setStatus(`${timingCopy.returnOrder} ${timingCopy.returnOrderStrong}`);return;} const form=new FormData(e.currentTarget); if(String(form.get('companyWebsite')||'').trim())return; if(phoneCountryCodeMissing&&!phoneCountryConfirmedRef.current){setPhoneConfirmPending(true);setStatus('');return;}
    ...
    void fetch('/api/booking',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(details),keepalive:true}).then(async r=>{const result=await r.json().catch(()=>null) as {email?:string}|null;if(result?.email==='sent')setStatus(t.requestEmail); if(!r.ok||result?.email==='send-failed')setStatus(t.requestEmailFail)}).catch(()=>setStatus(t.requestReady)).finally(()=>{submittingRef.current=false;phoneCountryConfirmedRef.current=false;setIsSubmitting(false)});
  }
  return <div id="booking-form" className={`booking-card${compact?' booking-card-compact':''}`}><div className="section-head booking-head"><div className="kicker">{t.kicker}</div><h2>{t.heading}</h2><p>{t.intro}</p></div>
    <form ref={formRef} onSubmit={submit}><input className="hp-field" tabIndex={-1} autoComplete="off" name="companyWebsite" aria-hidden="true"/><div className="form-grid">
      ...
      {!companyArrangedPickup&&(firstTiming==='urgent'||firstTiming==='short-notice')&&<div className={`field full short-notice-warning${firstTiming==='urgent'?' urgent':''}`}>⚠️ <span>{firstTiming==='urgent'?timingCopy.urgent:timingCopy.short} <strong>{firstTiming==='urgent'?timingCopy.urgentStrong:timingCopy.shortStrong}</strong></span></div>}
      {!expanded&&<div className="field full"><button className="btn booking-continue" type="button" disabled={!firstStageReady} aria-disabled={!firstStageReady} onClick={()=>{if(firstStageReady)setExpanded(true)}}>{t.continue} · €{total}</button><div className="form-note">{firstStageReady?t.expandReady:companyArrangedPickup?expandMissingWithoutTime[locale]:t.expandMissing}</div></div>}
      {expanded&&<>
        ...
        <div className="field full contact-methods-row"><div className="field"><label>{locale==='es'?'Número de contacto (WhatsApp si está disponible)':locale==='pt-BR'?'Número de contato (WhatsApp se disponível)':locale==='ko'?'연락처 전화번호 (WhatsApp 사용 시 해당 번호)':'連絡先電話番号（WhatsAppがある場合はその番号）'}</label><input id={`localized-whatsapp-${locale}-${compact?'compact':'full'}`} type="tel" value={whatsapp} onChange={e=>updateWhatsApp(e.target.value)} placeholder={t.whatsappPlaceholder} autoComplete="tel" required/></div><div className="field"><label>{contact.emailLabel}</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@example.com" autoComplete="email"/></div><div className="form-note contact-fallback-note">{contact.fallback}</div>{phoneCountryCodeMissing&&<div className="short-notice-warning phone-country-warning" role="status">⚠️ <span><strong>{contact.phoneCountryWarning}</strong></span></div>}{phoneConfirmPending&&<div className="phone-country-confirm" role="alert"><div><strong>{contact.phoneConfirmTitle}</strong><span>{contact.phoneConfirmText} <b>{whatsapp}</b></span></div><div className="phone-country-actions"><button type="button" className="btn btn-secondary" onClick={()=>{setPhoneConfirmPending(false);document.getElementById(`localized-whatsapp-${locale}-${compact?'compact':'full'}`)?.focus()}}>{contact.phoneFix}</button><button type="button" className="btn btn-primary" onClick={continueWithCurrentPhone}>{contact.phoneContinue}</button></div></div>}</div>
