...
export function LocalizedBookingForm({ locale, compact=false, initialAirport='kayseri', initialDirection='airport-hotel', initialTown='' }: { locale:NewLocale; compact?:boolean; initialAirport?:Airport; initialDirection?:Direction; initialTown?:string }) {
  ...
  const today=useMemo(()=>todayInIstanbul(),[]); const firstStageReady=Boolean(destination && hotel.trim() && firstTransferDate && firstTransferTime); const sameDayBooking=Boolean(firstTransferDate===today && firstTransferDate);
  ...
  return <div className={`booking-card${compact?' booking-card-compact':''}`}><div className="section-head booking-head"><div className="kicker">{t.kicker}</div><h2>{t.heading}</h2><p>{t.intro}</p></div>
    <form onSubmit={submit}><input className="hp-field" tabIndex={-1} autoComplete="off" name="companyWebsite" aria-hidden="true"/><div className="form-grid">
      ...
      <div className="field full"><label htmlFor={`passengers-loc-${compact?'c':'f'}`}>{t.passengerCount}</label><input id={`passengers-loc-${compact?'c':'f'}`} type="number" min="1" max={transferType==='private'?(vehicle==='vito'?5:16):16} value={passengers} onChange={e=>{const max=transferType==='private'?(vehicle==='vito'?5:16):16;setPassengers(Math.min(max,Math.max(1,Number(e.target.value)||1)))}} required/></div>
      ...
      <div className="field"><label>{firstDateLabel}</label><input type="date" min={today} value={firstTransferDate} onChange={e=>setFirstTransferDate(e.target.value)} required/></div><div className="field"><label>{firstTimeLabel}</label><input type="time" value={firstTransferTime} onChange={e=>setFirstTransferTime(e.target.value)} required/></div>
