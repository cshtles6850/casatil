...
export const metadata: Metadata = {
  title: { absolute: '卡帕多奇亚机场接送｜ASR / NAV €15 拼车与私人包车' },
  description: '卡帕多奇亚机场接送：从开塞利 ASR 或内夫谢希尔 NAV 前往格雷梅、于尔居普、乌奇希萨尔、阿瓦诺斯、奥塔西萨和恰武辛。拼车 €15/人，私人 Vito / Sprinter 可选。',
  keywords: ['卡帕多奇亚机场接送','卡帕多奇亚机场拼车','卡帕多奇亚机场班车','卡帕多奇亚接机','开塞利机场接送','内夫谢希尔机场接送','格雷梅机场接送','Cappadocia shuttle transfer'],
  alternates: { canonical: '/zh-cn', languages: { en: '/', 'zh-CN': '/zh-cn', 'x-default': '/' } },
  openGraph: { type: 'website', locale: 'zh_CN', title: '卡帕多奇亚机场接送｜€15 拼车', description: 'ASR / NAV 到卡帕多奇亚酒店，拼车 €15/人，私人 Vito / Sprinter 可选。', url: `${SITE.domain}/zh-cn`, images: [{ url: '/cappadocia-airport-shuttle-vito-sprinter.webp', width: 1200, height: 675, alt: '卡帕多奇亚机场接送 Vito 与 Sprinter' }] },
...
const homeFaq = [
  ...
  ['拼车覆盖哪些住宿区域？','格雷梅、于尔居普、乌奇希萨尔、阿瓦诺斯、恰武辛和奥塔西萨。穆斯塔法帕夏（Mustafapaşa）需要私人接送询价。'],
...
export default function ChineseHomePage() {
  ...
  return <main lang="zh-CN">
    ...
    <section className="section"><div className="container">
      ...
      <div className="cards">
        <Link href="/zh-cn/kayseri-airport-transfer" className="card card-link"><h3>开塞利机场 ASR</h3><p>格雷梅等六个酒店区的路线入口。Vito €90 · Sprinter €110。</p></Link>
        <Link href="/zh-cn/nevsehir-airport-transfer" className="card card-link"><h3>内夫谢希尔机场 NAV</h3><p>前往多个中心城镇公路更短。Vito €80 · Sprinter €90。</p></Link>
    ...
    <section className="section"><div className="container">
      ...
      <div className="route-groups">
        <div><h3>开塞利机场（ASR）</h3><div className="route-link-grid">{townKeys.map((key)=><Link key={key} href={`/zh-cn/kayseri-airport-to-${key}-transfer`}><strong>开塞利机场 → {zhTownNames[key]}</strong><span>{towns[key].distanceKayseri} · {towns[key].timeKayseri}</span></Link>)}</div></div>
        <div><h3>内夫谢希尔机场（NAV）</h3><div className="route-link-grid">{townKeys.map((key)=><Link key={key} href={`/zh-cn/nevsehir-airport-to-${key}-transfer`}><strong>内夫谢希尔机场 → {zhTownNames[key]}</strong><span>{towns[key].distanceNevsehir} · {towns[key].timeNevsehir}</span></Link>)}</div></div>
      </div>
    </div></section>
    ...
    <section className="section section-muted"><div className="container">
      ...
      <div className="route-groups">
        <div><h3>前往开塞利机场（ASR）</h3><div className="route-link-grid">{townKeys.map((key)=><Link key={key} href={`/zh-cn/${key}-to-kayseri-airport-transfer`}><strong>{zhTownNames[key]} → 开塞利机场</strong><span>{towns[key].distanceKayseri} · {towns[key].timeKayseri}</span></Link>)}</div></div>
        <div><h3>前往内夫谢希尔机场（NAV）</h3><div className="route-link-grid">{townKeys.map((key)=><Link key={key} href={`/zh-cn/${key}-to-nevsehir-airport-transfer`}><strong>{zhTownNames[key]} → 内夫谢希尔机场</strong><span>{towns[key].distanceNevsehir} · {towns[key].timeNevsehir}</span></Link>)}</div></div>
      </div>
    </div></section>

    <section className="section"><div className="container">
      <div className="section-head"><div className="kicker">住宿区域</div><h2>不是把六个城镇写成同一页模板</h2><p>格雷梅的洞穴酒店坡路、乌奇希萨尔的城堡高差、阿瓦诺斯的河两岸分布、于尔居普的老城、奥塔西萨和恰武辛的村路，都需要不同的接送说明。</p></div>
      <div className="cards">{townKeys.map((key)=><Link className="card card-link" href={`/zh-cn/${key}-airport-transfer`} key={key}><h3>{zhTownNames[key]}机场接送</h3><p>比较 ASR / NAV、酒店道路、当地景点、住宿名称与返程安排。</p></Link>)}</div>
    </div></section>
